...
namespace lap2_21521327_GiaQUan
{
    public partial class bai4 : Form
    {
        ...
        void nhap(BinaryWriter br, List<string> list)
        {
            ...
            if (txt_phone.TextLength != 10 && txt_phone.Text[0] != 0)
            {
                MessageBox.Show("VUI LÒNG NHẬP SỐ ĐIỆN THOẠI CHÍNH XÁC", "THÔNG BÁO", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            else
            {
                if (float.Parse(txt_1.Text) > 10 || float.Parse(txt_1.Text) < 0 || float.Parse(txt_2.Text) < 0 || float.Parse(txt_2.Text) > 10 || float.Parse(txt_3.Text) < 0 || float.Parse(txt_3.Text) > 10)
                {
                    MessageBox.Show("VUI LÒNG NHẬP ĐIỂM LỚN HƠN 0 VÀ NHỎ HƠN 11", "THÔNG BÁO", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
                else
                {
